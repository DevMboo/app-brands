<div class="min-h-screen w-full bg-slate-100 rounded-s-2xl">
        ...
        <nav class="flex justify-between gap-2 relative mb-3">
            <div>
                <ul class="text-gray-700 flex">
                    <li>
                        <a href="#" @click="$dispatch('show-created-unity')" class="text-sm w-full flex gap-1 items-center px-4 py-2 bg-transparent hover:bg-slate-200 rounded-lg">          
                    ...
                    <li>
                        <a href="#" @click="$dispatch('show-historic-unity')" class="text-sm w-full flex gap-2 px-4 py-2 bg-transparent hover:bg-slate-200 rounded-lg">
                            Histórico geral
                        </a>
                    </li>
                </ul>
            ...
            <livewire:pages.components.export 
                ...
                :fk="['flag_id' => 'name']"
                :itWith="'flag'"
                :title="'Exportação de unidades'"
                :whereBy="'name_fantasy'"
            />
        ...
        <div class="w-full">
            <div class="grid grid-cols-4 gap-4">
                @forelse ($units as $unity)
                    <livewire:pages.unity.components.card-unity :unityId="$unity->id" wire:key="{{ $unity->id }}" />
                @empty
                    <div class="col-span-4 text-center">
                        <img src="{{ asset('assets/images/empty.svg')}}" class="w-60 h-60 mx-auto" alt="Empty icon">
                        <em class="text-neutral-500 text-center">Nenhuma unidade foi criada...</em>
                    </div>
                @endforelse
    ...
    <!-- MODAL CREATED UNITY START -->
    <livewire:pages.unity.components.modal-unity-create />
    <livewire:pages.unity.components.modal-unity-delete />
    <livewire:pages.unity.components.modal-unity-edit />
    <livewire:pages.unity.components.modal-unity-view />
    <!-- MODAL CREATED UNITY START -->
</div>

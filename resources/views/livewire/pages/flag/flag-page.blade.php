<div class="min-h-screen w-full bg-slate-100 rounded-s-2xl">
        ...
        <nav class="flex justify-between gap-2 relative mb-3">
            <div>
                <ul class="text-gray-700 flex">
                    <li>
                        <a href="#" @click="$dispatch('show-created-flag')" class="text-sm w-full flex gap-1 items-center px-4 py-2 bg-transparent hover:bg-slate-200 rounded-lg">          
                    ...
                    <li>
                        <a href="#" @click="$dispatch('show-historic-flag')" class="text-sm w-full flex gap-2 px-4 py-2 bg-transparent hover:bg-slate-200 rounded-lg">
                            Histórico geral
                        </a>
                    </li>
                </ul>
            ...
            <livewire:pages.components.export 
                modelClass="App\Models\Flag"
                :aliases="[
                    'id' => '#',
                    'name' => 'Nome da bandeira',
                    'group_economy_id' => 'Grupo Econômico',
                    'created_at' => 'Data de criação',
                    'updated_at' => 'Última atualização'
                ]" 
                :fk="['group_economy_id' => 'name']"
                :itWith="'group'"
                :title="'Exportação de Bandeiras'"
            />
        ...
        <div class="w-full">
            <div class="grid grid-cols-4 gap-4">
                @forelse ($flags as $flag)
                    <livewire:pages.flag.components.card-flag :flagId="$flag->id" wire:key="{{ $flag->id }}" />
                @empty
                    <div class="col-span-4 text-center">
                        <img src="{{ asset('assets/images/empty.svg')}}" class="w-60 h-60 mx-auto" alt="Empty icon">
                        <em class="text-neutral-500 text-center">Nenhuma bandeira foi criada...</em>
                    </div>
                @endforelse
    ...
    <livewire:pages.flag.components.modal-flag-create />
    <livewire:pages.flag.components.modal-flag-edit />
    <livewire:pages.flag.components.modal-flag-view />
    <livewire:pages.flag.components.modal-flag-delete />
    <!-- MODAL CREATED FLAG END -->
</div>
